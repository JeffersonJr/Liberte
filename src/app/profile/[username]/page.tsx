...
export default function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
            ...
            try {
                ...
                if (profileData) {
                    const [userPosts, followingStatus] = await Promise.all([
                        ...
                        liberteIsFollowing(currentUserId, profileData.id)
                    ]);
                    setPosts(userPosts || []);
    ...
    const handleFollowToggle = async () => {
        ...
        try {
            if (isFollowing) {
                await liberteUnfollowUser(currentUserId, profile.id);
                setIsFollowing(false);
            } else {
                await liberteFollowUser(currentUserId, profile.id);
                setIsFollowing(true);
            }
        } catch (error) {
    ...
    if (!isLoading && !profile) {
        return (
            <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-4">
                ...
                <Link href="/" className="text-zinc-100 hover:underline">Go back home</Link>
            </div>
        );
    }
                ...
                <header className="glass sticky top-0 z-50 px-4 py-2 backdrop-blur-xl flex items-center gap-6">
                    ...
                    <div>
                        <h1 className="font-bold text-lg text-zinc-100">
                            {isLoading ? "Profile" : (profile?.full_name || username)}
                        </h1>
                        <p className="text-zinc-500 text-xs">
                            {isLoading ? "..." : `${posts.length} publications`}
                        </p>
                    </div>
                </header>
